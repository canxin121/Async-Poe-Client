...
class Async_Poe_Client:
    ...
    async def get_basedata(self):
        ...
        try:
            self.bot_list_url = f'https://poe.com/_next/data/{self.next_data["buildId"]}/index.json'
            ...
            raise ValueError("Failed to extract 'viewer' or 'user_id' from 'next_data'.") from e  # noqa: E501
        ...
        try:
            ...
            self.formkey = "".join(formkey_list)[:-1]  # by @aditiaryan and @ading2210
            self.client.headers.update({"poe-formkey": self.formkey})
        except AttributeError as e:
            raise ValueError("Failed to extract 'formkey' from the response text.") from e  # noqa: E501
    ...
    def get_websocket_url(self, channel=None):
        ...
        return f'ws://{self.ws_domain}.tch.{channel["baseHost"]}/up/{channel["boxName"]}/updates' + query  # noqa: E501
    ...
    async def get_botdata(self, url_botname: str):
        """
        ...
        """
        url = f'https://poe.com/_next/data/{self.next_data["buildId"]}/{url_botname}.json'
        try:
            response = await self.client.get(url, timeout=3)
            chat_data = response.json()["pageProps"]["data"]["chatOfBotHandle"]
            return chat_data
        except Exception as e:
            raise ValueError(f"Failed to get bot chat_data from {url}") from e

    async def get_bot_configuration(self, url_botname: str) -> dict:
        """
        ...
        try:
            ...
            raise ValueError(f"Failed to get bot info from {url}") from e

    async def save_botdata(self, url_botname: str):
        """
    ...
    async def get_bots(self):
        ...
        if "availableBotsConnection" not in self.viewer:
            raise RuntimeError("Failed to get_bots: Invalid token or no bots are available.")
    ...
    async def send_query(self, query_name: str, variables: dict):
        """
        ...
        query_headers = {**self.client.headers,
        ...
        while retry:
            try:
                if query_name == "recv":
                    await self.client.post(GQL_RECV_URL, headers=query_headers, data=data)
                    return None
                else:
                    response = await self.client.post(GQL_URL, data=data, headers=query_headers)
                json_data = response.json()
                ...
                detail_error = e
                logger.error(
                    f"Failed to sending query:{query_name},error:{detail_error}. (retry: {retry}/5)")  # noqa: E501
                retry -= 1
        raise Exception(f"Too much error when sending query:{query_name},error:{detail_error}") from detail_error

    async def subscribe(self):
        """
        This function is used to simulate a human user opening the website by subscribing to certain mutations.

        The function sends a SubscriptionsMutation query containing two subscriptions: "messageAdded" and "viewerStateUpdated".
        ...
        try:
            ...
            logger.info("Succeeded to subscribe to mutations")
        except Exception as e:
            raise Exception("Failed to subscribe by sending SubscriptionsMutation") from e  # noqa: E501

    async def create_bot(self, handle: str, prompt: str, display_name: Optional[str] = None,
                         base_model: str = "chinchilla", description: Optional[str] = "",
                         intro_message: Optional[str] = "", api_key: Optional[str] = None,
                         api_bot: Optional[bool] = False, api_url: Optional[str] = None,
                         prompt_public: Optional[bool] = True, pfp_url: Optional[str] = None,
                         linkification: Optional[bool] = False,
                         markdown_rendering: Optional[bool] = True, suggested_replies: Optional[bool] = False,
        ...
        pfp_url (str, optional): The profile picture URL for the new bot. If not provided, it will be set to None.
        ...
        """
        result = await self.send_query("PoeBotCreateMutation", {
            "model": base_model,
    ...
    async def edit_bot(self, url_botname: str, handle: str = None, prompt: Optional[str] = None, display_name=None,
                       # noqa: E501
                       base_model="chinchilla",
                       description="",
                       intro_message="", api_key=None, api_url=None, is_private_bot=None,
                       prompt_public=None, profile_picture_url=None, linkification=None,
                       markdown_rendering=None, suggested_replies=None, temperature=None):
        """
        ...
        """
        botinfo = await self.get_bot_configuration(url_botname)
        botinfo = botinfo['data']['bot']

        result = await self.send_query("PoeBotEditMutation", {
    ...
    async def send_message(self, url_botname: str, question: str, with_chat_break: bool = False):  # noqa: E501
        """
        ...
        message_data = await self.send_query("SendMessageMutation", {
            "bot": handle,
            "query": question,
    ...
    async def send_recv(self, url_botname: str, last_text: str, bot_message_id: str, human_message_id: str):
        """
    ...
    async def ask(self, url_botname: str, question: str, with_chat_break: bool = False, plain: bool = False) \
            -> Union[str, dict]:  # noqa: E501
        """
        ...
        plain (bool, optional): If set to True, the response will be plain text. If False, the response will be a dictionary. Default is False.
        ...
        """
        human_message_id = await self.send_message(url_botname, question, with_chat_break)
        retry = 3
            ...
            try:
                ...
                if messages[-1]['node']['messageId'] <= human_message_id:
                    ...
                    if messages[-1]['node']['text']:
                        await self.send_recv(url_botname, messages[-1]['node']['text'],
                                             messages[-1]['node']['messageId'], human_message_id)
                        if plain:
                        ...
                        if differ > 180:
                            raise ValueError(f"Timed out while getting message from {url_botname}")
                        continue
            except Exception as e:
                logger.error(f"Failed to getting message from {url_botname}, error:{str(e)}")  # noqa: E501
        raise ValueError(f"Failed to getting message from {url_botname} too many times")

    async def ask_stream(self, url_botname: str, question: str, with_chat_break: bool = False,  # noqa: E501
                         suggest_able: bool = False) -> AsyncGenerator:
        """
        ...
        async with connect(self.get_websocket_url(), user_agent_header=ua) as ws:
            human_message_id = await self.send_message(url_botname, question, with_chat_break)
            ...
            while True:
                try:
                    ...
                    message = json.loads(json.loads(raw_data)["messages"][-1])["payload"]["data"][  # noqa: E501
                        "messageAdded"]
                    if message['messageId'] > human_message_id:
                        plain_text = message["text"][len(last_text):]
                        last_text = message["text"]
                        if self.bots[url_botname]['defaultBotObject']['hasSuggestedReplies'] and suggest_able:
                    ...
                    raise Exception(f"Failed to get message from {url_botname} through websockets:{str(e)}")from e
            await self.send_recv(url_botname, last_text, message['messageId'], human_message_id)

    async def send_chat_break(self, url_botname: str):
        """
        ...
        This function sends an "AddMessageBreakMutation" query to the specified bot. If the bot data is not already saved, it will be saved first.
        ...
        if url_botname not in self.bots.keys():
            self.bots[url_botname] = await self.get_botdata(url_botname)
        result = await self.send_query("AddMessageBreakMutation", {
            "chatId": self.bots[url_botname]["chatId"]})
        logger.info(f'Succeed to chat break to {url_botname}:{result["data"]["messageBreakEdgeCreate"]["message"]}')
        return
    ...
    async def delete_messages(self, message_ids: Union[list, int]):
        ...
        This function sends a "DeleteMessageMutation" query with the message IDs that need to be deleted, and then logs a success message upon completion.
        ...
        await self.send_query("DeleteMessageMutation", {
            "messageIds": message_ids
        })
        logger.info(f"Succeed to deleting messages: {message_ids}")

    async def get_message_history(self, url_botname, count: Optional[int] = None,
        ...
        if not (count or get_all):
            raise TypeError("Please provide at least one of the following parameters: del_all=<bool>, count=<int>")
        messages = self.bots[url_botname]['messagesConnection']['edges']
        if len(messages) == 0:
            logger.error(f"Failed to get message history of {url_botname}: No messages found with {url_botname}")
            return []
        cursor = messages[0]['cursor']
        if not get_all and count <= len(messages):
            return messages[-count:]
        while get_all or (count > len(messages)):
            result = await self.send_query("ChatListPaginationQuery", {
                "count": 20,
                "cursor": cursor,
                "id": self.bots[url_botname]["id"]
            })
            ...
            if len(previous_messages) == 0:
                if not all:
                    logger.warning(f"Only {str(len(messages))} history messages found with {url_botname}")  # noqa: E501
                break
    ...
    async def delete_bot_conversation(self, url_botname: str, count: Optional[int] = None,
    ...
    async def delete_all_conversations(self):
        ...
        This function sends a "DeleteUserMessagesMutation" query and then logs success message upon completion.
        ...
        logger.info("Succeed to purge all conversations")
